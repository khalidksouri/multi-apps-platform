...
const Math4Kids = () => {
  ...
  const createParticles = useCallback((type) => {
    const newParticles = [];
  ...
  }, [userAnswer, currentQ.answer, level, streak, lives, currentQuestion, levelQuestions.length, playSound, createParticles, hapticFeedback, checkAchievements]);
  ...
  // Charger le score initial depuis localStorage
  useEffect(() => {
    const savedScore = localStorage.getItem('math4kids_highscore');
    if (savedScore) {
      // Optionnel : vous pouvez faire quelque chose avec le score sauvegardé
      console.log('Meilleur score sauvegardé:', savedScore);
    }
  }, []); // Pas de dépendance sur score

  // Sauvegarder le score dans localStorage (CORRECTION DE L'ERREUR)
  useEffect(() => {
    const savedScore = localStorage.getItem('math4kids_highscore');
    if (!savedScore || score > parseInt(savedScore)) {
      localStorage.setItem('math4kids_highscore', score.toString());
    }
  }, [score]); // Maintenant score est bien inclus dans les dépendances

...
WORKSPACE_DIR="/Users/khalidksouri/Desktop/multi-apps-platform/apps"
...
# Démarrer une application (basé sur votre script qui marche)
start_app() {
    local app_name=$1
...
start_all_apps() {
    ...
    if [ ! -d "$WORKSPACE_DIR" ]; then
        echo -e "${RED}❌ Workspace non trouvé${NC}"
        exit 1
    fi
...
repair_all() {
    ...
    echo -e "${GREEN}✅ Réparations terminées${NC}"
    echo ""
}
...
main() {
    case "${1:-help}" in
        ...
        *)
            ...
            echo ""
            echo -e "  ${GREEN}$0 start${NC}      - Démarrer toutes les applications"
            echo -e "  ${YELLOW}$0 status${NC}     - Voir le statut"
            echo -e "  ${CYAN}$0 repair-all${NC} - Réparer toutes les applications"
            echo -e "  ${BLUE}$0 repair [app]${NC} - Réparer une application"
            echo ""

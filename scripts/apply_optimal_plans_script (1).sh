...
# Fonction pour identifier et modifier la page existante
update_existing_page() {
    log_info "Recherche et mise à jour de la page existante..."
    ...
    if [[ -z "$FOUND_PAGE" ]]; then
        log_error "Aucune page trouvée. Créez une page manuellement."
        return 1
    fi
    
    # Sauvegarder le fichier original
    cp "$FOUND_PAGE" "$BACKUP_DIR/$(basename $FOUND_PAGE).original"
...
EOF
    ...
    log_success "Page existante mise à jour avec les plans optimaux"
}
...
main() {
    ...
    echo "1. Rechargez votre page (F5 ou Ctrl+R)"
    echo "2. Vérifiez que les nouveaux plans apparaissent"
    echo "3. Testez les sélecteurs de période"
    echo "4. Lancez les tests: npm run test:optimal-plans"
    echo ""
    log_info "📁 Sauvegarde disponible dans: $BACKUP_DIR"
    log_warning "Si la page ne change pas, vérifiez le cache du navigateur ou redémarrez le serveur de développement"
}

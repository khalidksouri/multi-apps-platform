...
echo "🚀 CORRECTION COMPLÈTE MATH4CHILD - RÉSOLUTION DES ERREURS TYPESCRIPT"
echo "=================================================================="

# Configuration
APP_DIR="apps/math4child"
PAGE_FILE="$APP_DIR/src/app/page.tsx"
BACKUP_FILE="${PAGE_FILE}.corrected_backup_$(date +%Y%m%d_%H%M%S)"

# Vérification de l'existence du répertoire
if [[ ! -d "$APP_DIR" ]]; then
    echo "❌ Erreur: Le répertoire $APP_DIR n'existe pas"
    echo "📍 Exécutez ce script depuis la racine du workspace"
    exit 1
fi

# Vérification de l'existence du fichier
if [[ ! -f "$PAGE_FILE" ]]; then
    echo "❌ Erreur: Le fichier $PAGE_FILE n'existe pas"
    exit 1
fi
...
echo "🔧 Application de la correction TypeScript..."
...
echo "✅ Fichier page.tsx remplacé avec succès"
...
if npx tsc --noEmit src/app/page.tsx 2>/dev/null; then
    echo "✅ Compilation TypeScript réussie !"
    echo ""
    echo "🎉 CORRECTION RÉUSSIE !"
    echo "======================================================="
    echo "✅ Modules manquants supprimés (@/components/language/LanguageDropdown, @/lib/optimal-payments)"
    echo "✅ Code TypeScript propre et auto-suffisant"
    echo "✅ Interface utilisateur complète avec:"
    echo "   • Sélecteur multilingue fonctionnel"
    echo "   • Progression des niveaux 1-5"
    echo "   • Plans d'abonnement optimisés (6.99€, 4.99€, 24.99€)"
    ...
    echo ""
    echo "📁 Sauvegarde de l'ancien fichier: $BACKUP_FILE"
else

...
print_info "Réinstallation des dépendances avec résolution des conflits..."
npm install --legacy-peer-deps

# Si échec, essayer avec force
if [ $? -ne 0 ]; then
    print_warning "Première tentative échoué, utilisation de --force..."
    npm install --force
    
    # Si toujours en échec, corriger les versions Capacitor
    if [ $? -ne 0 ]; then
        print_warning "Correction des versions Capacitor..."
        npm install @capacitor/core@^7.0.0 @capacitor/app@^7.0.1 --legacy-peer-deps
        npm install --legacy-peer-deps
    fi
fi

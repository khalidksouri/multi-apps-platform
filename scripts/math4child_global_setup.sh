...
# ===================================================================
# 🚀 MATH4CHILD - SCRIPT GLOBAL CORRIGÉ ET ROBUSTE
# Installation complète, propre et robuste de Math4Child
# Domaine: www.math4child.com | Développé par GOTEST
# Version corrigée basée sur les logs d'erreur
# ===================================================================
...
readonly LOG_FILE="${PROJECT_ROOT}/math4child_setup.log"
...
# Fonction pour créer un fichier SEULEMENT s'il n'existe pas
create_file_if_not_exists() {
    ...
    if [ ! -f "$file_path" ]; then
        ...
        print_warning "Fichier existant conservé: $(basename "$file_path")"
    fi
}
...
print_header "🚀 MATH4CHILD - SETUP GLOBAL CORRIGÉ"
...
echo "=== MATH4CHILD SETUP LOG ===" > "$LOG_FILE"
log "Début du setup Math4Child CORRIGÉ"
...
print_step "🔍 ÉTAPE 1: VÉRIFICATIONS PRÉALABLES RENFORCÉES"
...
if command_exists node; then
    ...
    # Vérifier la compatibilité (Node 18+ requis, mais accepter Node 20+)
...
# ===================================================================
# 🧹 ÉTAPE 2: NETTOYAGE ULTRA-COMPLET ET FORCÉ
# ===================================================================

print_step "🧹 ÉTAPE 2: NETTOYAGE ULTRA-COMPLET ET FORCÉ"

# Arrêter TOUS les processus Node.js/Next.js de manière agressive
...
if [ -d "$MATH4CHILD_DIR" ]; then
    ...
    chmod -R 755 "$MATH4CHILD_DIR" 2>/dev/null || true  # Forcer les permissions
    rm -rf "$MATH4CHILD_DIR"
    print_success "Ancienne installation COMPLÈTEMENT supprimée"
fi

# Nettoyer TOUS les caches de manière agressive
...
# Supprimer les caches système de manière agressive
...
# ===================================================================
# 📁 ÉTAPE 3: CRÉATION DE LA STRUCTURE PROPRE
# ===================================================================

print_step "📁 ÉTAPE 3: CRÉATION DE LA STRUCTURE PROPRE"

# Créer le répertoire principal avec permissions correctes
...
print_success "Structure de répertoires créée proprement"
echo ""

# ===================================================================
# 📄 ÉTAPE 4: CRÉATION DES FICHIERS SOURCES FORCÉE
# ===================================================================

print_step "📄 ÉTAPE 4: CRÉATION DES FICHIERS SOURCES (FORCÉE)"
...
# 9. Modal de pricing (version simplifiée)
...
print_success "Fichiers sources créés (FORCÉS)"
echo ""

# ===================================================================
# ⚙️ ÉTAPE 5: CONFIGURATION NEXT.JS COMPATIBLE (FORCÉE)
# ===================================================================

print_step "⚙️ ÉTAPE 5: CONFIGURATION NEXT.JS COMPATIBLE (FORCÉE)"

# Package.json CORRIGÉ avec versions compatibles Node 24+
...
# ESLint config CORRIGÉ (version 9+)
...
print_success "Configuration Next.js créée (COMPATIBLE Node 24+)"
echo ""

# ===================================================================
# 📦 ÉTAPE 6: INSTALLATION ROBUSTE DES DÉPENDANCES
# ===================================================================

print_step "📦 ÉTAPE 6: INSTALLATION ROBUSTE DES DÉPENDANCES"

# Aller dans le répertoire Math4Child
cd "$MATH4CHILD_DIR"

print_info "Installation des dépendances npm (mode robuste)..."
...
# ===================================================================
# 🧪 ÉTAPE 7: TESTS ET VALIDATION ROBUSTES
# ===================================================================

print_step "🧪 ÉTAPE 7: TESTS ET VALIDATION ROBUSTES"

# Test de compilation TypeScript avec gestion des erreurs
...
# Test rapide de Next.js (build sans optimisation)
...
# ===================================================================
# 📊 ÉTAPE 8: RAPPORT FINAL ROBUSTE
# ===================================================================

print_step "📊 ÉTAPE 8: RAPPORT FINAL ROBUSTE"
...
# ===================================================================
# 🎉 RAPPORT FINAL ROBUSTE
# ===================================================================

print_header "🎉 RAPPORT FINAL DE L'INSTALLATION ROBUSTE"
...
if [ $overall_score -ge 85 ]; then
    echo -e "${GREEN}✅ Installation EXCELLENTE et ROBUSTE !${NC}"
...
print_header "🚀 MATH4CHILD - SETUP GLOBAL COMPLET"
echo -e "${CYAN}🌍 Domaine: www.math4child.com | Développé par GOTEST${NC}"
echo -e "${CYAN}📧 Contact: gotesttech@gmail.com | SIRET: 53958712100028${NC}"
echo ""
...
print_step "🧹 ÉTAPE 2: NETTOYAGE COMPLET"

# Arrêter les processus Node.js/Next.js
print_info "Arrêt des processus Node.js/Next.js..."
...
if [ -d "$MATH4CHILD_DIR" ]; then
    ...
    rm -rf "$MATH4CHILD_DIR"
    print_success "Ancienne installation supprimée"
fi

# Nettoyer les caches npm
print_info "Nettoyage des caches npm..."
npm cache clean --force >/dev/null 2>&1 || true
...
if npm install --prefer-offline --no-audit --silent; then
    print_success "Dépendances installées avec succès"
else
...
for dep in "${critical_deps[@]}"; do
    if [ -d "node_modules/$dep" ]; then
        ...
        print_warning "$dep manquant - tentative de réinstallation..."
        npm install "$dep" --save --silent || print_error "Échec installation $dep"
    fi
done
...
if timeout 60s npm run build >/dev/null 2>&1; then
    ...
    print_warning "Build Next.js échoué ou trop long (peut être normal lors du premier build)"
fi
...
if [ -d "node_modules" ] && [ "$(ls -A node_modules)" ]; then
    print_success "node_modules peuplé"
else
    print_error "node_modules vide ou manquant"
fi

# Calculer le score de réussite
total_files=${#required_files[@]}
missing_count=${#missing_files[@]}
success_score=$(( (total_files - missing_count) * 100 / total_files ))
...
echo -e "${BLUE}📍 Répertoire: $MATH4CHILD_DIR${NC}"
echo -e "${BLUE}📊 Score de réussite: $success_score%${NC}"

if [ $success_score -ge 90 ]; then
    echo -e "${GREEN}✅ Installation EXCELLENTE !${NC}"
    echo -e "${GREEN}🚀 Application prête à être lancée${NC}"
elif [ $success_score -ge 70 ]; then
    echo -e "${YELLOW}⚠️ Installation CORRECTE avec quelques problèmes mineurs${NC}"
else
    echo -e "${RED}❌ Installation INCOMPLÈTE - des problèmes ont été détectés${NC}"
fi
...
echo -e "${GREEN}✅ Interface utilisateur moderne${NC}"
echo -e "${GREEN}✅ Navigation responsive${NC}"
echo -e "${GREEN}✅ Configuration Next.js/TypeScript${NC}"
echo -e "${GREEN}✅ Support complet RTL${NC}"
...
# Sauvegarder le rapport dans le log
log "=== RAPPORT FINAL ==="
log "Score de réussite: $success_score%"
log "Fichiers manquants: ${missing_files[*]}"
log "Installation terminée à: $(date)"

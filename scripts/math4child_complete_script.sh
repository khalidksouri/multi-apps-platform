...
initialize_project() {
    ...
    if [ -d "math4child-optimal" ]; then
        ...
        rm -rf math4child-optimal
    fi
    
    print_info "Création projet Next.js optimisé..."
    npx create-next-app@latest math4child-optimal \
        --typescript \
        --tailwind \
        --eslint \
        --app \
...
build_and_validate() {
    ...
    if npm run test:optimal -- --timeout 15000 --max-failures=1; then
        print_success "Tests de base réussis !"
    else
        print_warning "Certains tests ont échoué - normal en développement"
    fi
}
...
show_instructions() {
    ...
    echo -e "${GREEN}✅ STRUCTURE API CORRIGÉE :${NC}"
    echo "   • Dossiers API créés automatiquement"
    echo "   • Routes webhooks complètes"
    ...
    print_success "Math4Child Optimal Complet est opérationnel ! 🚀"
}

...
# Installation directe du système de navigation unifié
#===============================================================================
...
show_banner() {
    ...
    cat << 'EOF'
╔══════════════════════════════════════════════════════════════════════════════╗
║               🧭 MATH4CHILD - NAVIGATION INSTALLER                          ║
...
go_to_math4child() {
    if [ -d "apps/math4child" ]; then
        cd apps/math4child
        log_message "Trouvé: apps/math4child"
    else
        log_message "ERROR: apps/math4child non trouvé"
        exit 1
    fi
}
...
# Créer la structure
create_structure() {
    log_message "Création de la structure..."
...
create_navigation() {
    log_message "Création du composant Navigation..."
...
# Créer le contexte
create_context() {
    log_message "Création du contexte de langue..."
...
update_layout() {
    log_message "Mise à jour du layout..."
...
# Mettre à jour tsconfig
update_tsconfig() {
    log_message "Mise à jour de la configuration TypeScript..."
...
# Créer les tests
create_tests() {
    log_message "Création des tests..."
...
main() {
    ...
    echo -e "${YELLOW}Continuer l'installation ? (y/N)${NC}"
    read -r response
    if [[ ! "$response" =~ ^[Yy]$ ]]; then

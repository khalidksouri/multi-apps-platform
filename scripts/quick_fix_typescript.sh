...
# =============================================================================
# 🔧 CORRECTION RAPIDE TYPESCRIPT MATH4CHILD
# =============================================================================

set -e

APP_DIR="$(pwd)/apps/math4child"
GREEN='\033[0;32m'
BLUE='\033[0;34m'
NC='\033[0m'
...
log "🔧 Correction rapide des erreurs TypeScript..."

# Corriger next.config.js
log "📝 Correction next.config.js..."
...
# Corriger le composant avec types appropriés
log "⚛️ Correction du composant avec types..."
...
success "Composant corrigé créé"
...
if [[ $? -eq 0 ]]; then
    ...
    echo ""
    echo -e "${BLUE}🌐 Ouvrez: http://localhost:3000${NC}"
    echo -e "${GREEN}🎉 Math4Child Premium avec 20+ langues est maintenant actif !${NC}"
else
    echo "❌ Build échoué"
    exit 1
fi
...
# Rendre exécutable
chmod +x quick_fix_typescript.sh

log "🚀 Lancement de la correction rapide..."
./quick_fix_typescript.sh
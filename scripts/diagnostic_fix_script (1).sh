...
fix_app_advanced() {
    ...
    # Obtenir le port pour cette application
    local app_port="${app_info%:*}"
    
    # Créer un script temporaire pour corriger le package.json
    cat > fix_package.js << EOF
const fs = require('fs');
try {
  ...
  if ('$app_name' === 'budgetcron' && !pkg.scripts.serve) {
    pkg.scripts.serve = 'PORT=3003 vue-cli-service serve --skip-plugins @vue/cli-plugin-eslint';
  }
  if ('$app_name' === 'multiai' && !pkg.scripts.dev) {
    pkg.scripts.dev = 'PORT=3005 next dev';
  }
  if (!pkg.scripts.start) {
    pkg.scripts.start = 'PORT=$app_port SKIP_PREFLIGHT_CHECK=true react-scripts start';
  }

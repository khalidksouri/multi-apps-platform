...
# Script d'intégration des changements UI visibles dans l'app Next.js
# Ce script modifie réellement les pages pour voir les changements
...
log_info "🔄 Intégration des changements UI dans l'application Next.js"

cd apps/math4child

# Backup de l'application actuelle
BACKUP_DIR="../backups/integration-$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"

if [ -d "src" ]; then
    cp -r src "$BACKUP_DIR/"
    log_info "📦 Backup de l'app actuelle créé"
fi

# 1. Trouver et modifier la page principale
log_info "🔍 Recherche de la page principale..."

# Chercher les fichiers de page possibles
PAGE_FILE=""
if [ -f "src/app/page.tsx" ]; then
    PAGE_FILE="src/app/page.tsx"
elif [ -f "src/pages/index.tsx" ]; then
    PAGE_FILE="src/pages/index.tsx"
elif [ -f "pages/index.tsx" ]; then
    PAGE_FILE="pages/index.tsx"
elif [ -f "app/page.tsx" ]; then
    PAGE_FILE="app/page.tsx"
else
    log_error "❌ Impossible de trouver la page principale"
    log_info "Fichiers trouvés dans src/ :"
    find src -name "*.tsx" -o -name "*.ts" | head -10 2>/dev/null || echo "Aucun fichier TypeScript trouvé"
    
    log_info "📁 Création de la structure App Router..."
    mkdir -p src/app
    PAGE_FILE="src/app/page.tsx"
fi

log_success "📄 Page principale trouvée/créée: $PAGE_FILE"

# 2. S'assurer que lucide-react est installé
...
# 3. Créer la page améliorée directement dans le bon format
log_info "🎨 Création de la page d'accueil améliorée..."
...
log_success "✅ Page d'accueil améliorée intégrée dans $PAGE_FILE"

# 4. Vérifier et créer layout.tsx si nécessaire (pour App Router)
if [[ "$PAGE_FILE" == *"src/app/"* ]]; then
    if [ ! -f "src/app/layout.tsx" ]; then
        log_info "📄 Création du layout.tsx pour App Router..."
...
export const metadata: Metadata = {
  ...
  description: 'L\'application n°1 pour apprendre les mathématiques en famille. Plus de 100k familles nous font confiance.',
}
...
EOF
        ...
        log_success "✅ Layout créé pour App Router"
    fi
    
    # Créer globals.css si nécessaire
...
EOF
        ...
        log_success "✅ Fichier globals.css créé"
    fi
fi
...
echo "  ✅ Page d'accueil complètement remplacée par la version améliorée"
echo "  ✅ Nouveau design moderne avec animations"
echo "  ✅ Sélecteur de langue avec recherche"
echo "  ✅ Modal de pricing responsive"
echo "  ✅ Gradients et effets visuels"
echo "  ✅ Layout et CSS configurés"
echo ""
echo "🚀 Pour voir les changements:"
echo "  npm run dev"
echo "  Puis ouvrez http://localhost:3000"
echo ""
echo "🔄 Si le serveur était déjà démarré, rechargez la page (Cmd+R ou Ctrl+R)"
echo ""
echo "📁 Backup de l'ancienne version dans: $BACKUP_DIR"
echo ""
log_success "Les changements sont maintenant visibles sur localhost:3000! 🎉"

cd ..
EOF

log_success "✅ Script d'intégration créé!"
